...
def log_sum_exp(x):
    ...
    return y.squeeze(1)  #降维为n*1

#从模型输出中选择与目标标签对应的预测结果
def class_select(logits, target):
    # in numpy, this would be logits[:, target]..模型输出，target目标标签
    ...
    if target.is_cuda:
        ...
        one_hot_mask = torch.autograd.Variable(torch.arange(0, num_classes)
                                               ...
                                               .repeat(batch_size, 1)
                                               .eq(target.data.repeat(num_classes, 1).t()))
    return logits.masked_select(one_hot_mask)  # masked-select选出logits输出和目标标签对应的结果
