...
from regularization.regularizer import L1Regularizer, L2Regularizer, ElasticNetRegularizer, \
...
TestBatchSize =  256#512
CommunicationEpoch = 100
Pariticpant_Params = {
    'loss_funnction' : 'SCE',
    'optimizer_name' : 'Adam',
    'learning_rate'  : 0.001
...
#Private_Nets_Name_List = ['ResNet12']
"""Dataset Setting"""
...
def update_model_via_private_data_old(network,private_epoch,private_dataloader,loss_function,optimizer_method,learing_rate,logger):
    if loss_function =='CE':
...
#----------------未修改代码----用来对比---------------------------------------------------------------------
            for batch_idx, (images, labels) in enumerate(private_dataloader):
                ...
                optimizer.zero_grad()
                participant_local_loss_batch_list.append(loss3.item())
                loss3.backward()
                optimizer.step()
                if epoch_index % 5 ==0 :#and  showLOSS != loss.item():
                    #showLOSS = loss.item()
                    ...
                    logger.info('Private Train : [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        batch_idx * len(images), len(private_dataloader.dataset),
                        100. * batch_idx / len(private_dataloader), loss3.item()))


    return network,participant_local_loss_batch_list

def update_model_via_private_data_new(network,private_epoch,private_dataloader,loss_function,optimizer_method,learing_rate,logger,curr_epoch):
    if loss_function =='CE':
        criterion = nn.CrossEntropyLoss()
    if loss_function =='SCE':
        criterion = SCELoss(alpha=0.1, beta=1.0, num_classes=10)
    ...
    for epoch_index in range(CommunicationEpoch):
        ...
        for participant_index in range(N_Participants):
            ...
            logger=logger)
                ...
                network,private_loss_batch_list = update_model_via_private_data_new(network=network,private_epoch=private_epoch,
            private_dataloader=train_dl_local,loss_function=Pariticpant_Params['loss_funnction'],
            optimizer_method=Pariticpant_Params['optimizer_name'],learing_rate=Pariticpant_Params['learning_rate'],
            logger=logger,curr_epoch = epoch_index)                
